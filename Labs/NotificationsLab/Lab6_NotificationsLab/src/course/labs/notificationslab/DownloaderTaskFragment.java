...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		Integer[] resIds = new Integer[MainActivity.sRawTextFeedIds.size()];
		MainActivity.sRawTextFeedIds.toArray(resIds);
	...
	public class DownloaderTask extends AsyncTask<Integer, Integer, String[]> {
	  ...
	  private String[] downloadTweets(Integer resourceIDS[]) {
						...
						public void onReceive(Context context, Intent intent) {
							...
							if (intent.getAction().equals(MainActivity.DATA_REFRESHED_ACTION)) {
								if (this.getResultCode() != MainActivity.IS_ALIVE) {
									...
									PendingIntent pi = PendingIntent.getActivity(mContext, 0, restartMainActivityIntent, PendingIntent.FLAG_UPDATE_CURRENT);
							...
							} else if (intent.getAction().equals(MainActivity.ALARM_ACTION)) {
								if (this.getResultCode() != MainActivity.IS_ALIVE) {
									MainActivity.setAlarm(mContext);									AlarmManager am = (AlarmManager) mContext.getSystemService(Context.ALARM_SERVICE);
								}
							}
						}
					}, null, 0, null, null);
		...
		@Override
		protected String[] doInBackground(Integer... resIds) {
			return downloadTweets(resIds);
		}
